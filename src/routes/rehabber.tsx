...
function RehabberDashboard() {
  ...
  const orderedCenters = location ? sortCentersByDistance(centers ?? [], location) : centers ?? [];
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="mx-auto max-w-2xl px-5 pb-16 pt-6"><Link to="/" search={{}} className="mb-5 inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Back</Link><p className="inline-flex rounded-lg bg-open-soft px-2.5 py-1 text-sm font-semibold text-open">Rehabber dashboard</p><h1 className="mt-1 font-display text-3xl">Manage your center</h1><p className="mt-2 text-[15px] leading-relaxed text-muted-foreground">Verify your work email before publishing capacity updates.</p>
      ...
      <label className="mt-6 block text-sm font-semibold" htmlFor="center">Your center</label><select id="center" value={centerId ?? ""} onChange={(event) => { setCenterId(event.target.value || undefined); setVerified(false); }} disabled={isLoading || isError} className="mt-2 w-full rounded-xl border border-input bg-card px-3 py-3 text-sm shadow-sm outline-none focus:ring-2 focus:ring-ring"><option value="">{isLoading ? "Loading centers..." : "Select your center"}</option>{orderedCenters.map((item) => <option key={item.id} value={item.id}>{item.name}{"distance" in item ? ` (${item.distance.toFixed(1)} mi)` : ""}</option>)}</select>
