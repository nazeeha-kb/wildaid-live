...
export function MapPanel({ location, locationStatus, onRetryLocation, individuals, places, mode, onModeChange, onPing, pingPending }: {
  ...
  return (
    <section className="relative h-72 w-full overflow-hidden rounded-2xl border border-border bg-muted shadow-sm sm:h-80">
      {mounted ? <Suspense fallback={<div className="h-full w-full animate-pulse bg-muted" />}><CapacityMap location={location} individuals={individuals} places={places} mode={mode} onPing={onPing} pingPending={pingPending} /></Suspense> : <div className="h-full w-full animate-pulse bg-muted" />}
