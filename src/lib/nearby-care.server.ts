...
export const getNearbyCarePlaces = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<NearbyCarePlace[]> => {
    ...
    const query = `[out:json][timeout:15];(nwr[amenity=veterinary](around:25000,${data.latitude},${data.longitude});nwr[healthcare=animal](around:25000,${data.latitude},${data.longitude});nwr[animal_shelter](around:25000,${data.latitude},${data.longitude});nwr["animal:wildlife_rehabilitation"="yes"](around:25000,${data.latitude},${data.longitude}););out center tags;`;
